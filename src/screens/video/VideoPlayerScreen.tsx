...
import { Video, VideoReadyForDisplayEvent, PlaybackStatus } from 'expo-video';
...
const VideoPlayerScreen: React.FC = () => {
  ...
  }, [playerState.isPlaying, playerState.currentTime, playerState.duration, updatePlayerState]);
  ...
  const onError = useCallback((error: string) => {
    ...
    if (playerState.retryCount < 3) {
      retryTimeoutRef.current = setTimeout(() => {
        ...
        if (videoRef.current) {
          videoRef.current.setSource({ uri: getCurrentVideoUrl() });
        }
      }, 2000);
    }
  ...
  const changeQuality = useCallback(async (quality: '480' | '720' | '1080') => {
    ...
    if (videoRef.current) {
      await videoRef.current.setSource({ uri: getCurrentVideoUrl() });
